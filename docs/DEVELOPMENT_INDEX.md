...
- v11.1 hard alignment refactor plan (2026-04-18): `docs/V11_1_HARD_ALIGNMENT_REFACTOR_PLAN_2026-04-18.md`
...
- `2026-04-18`: execution posture is now explicitly tightened again under `docs/V11_1_HARD_ALIGNMENT_REFACTOR_PLAN_2026-04-18.md`: `v11.1` is the only target state, compatibility retention is no longer a goal, physical structure must match logical ownership, and future work should prefer deletion plus owner replacement over another round of bridge/shim preservation.
...
- `2026-04-18`: gateway session mutation/control routes now consume `SessionTaskService` directly; `MainAgentSurfaceService` remains on the chat/session-read side, the gateway transport layer owns request-binding adaptation for the migrated mutation endpoints, and session-scoped compatibility methods on `AgentUserService` / `ModelUserService` are no longer part of the active HTTP mutation path.
...
- `2026-04-18`: `mini_agent.application.facades` now follows the same rule: legacy surface-facade symbols are lazy compatibility exports only, and `tests/test_surface_facade_boundary_hygiene.py` blocks active `src/` code from importing those surface facade entrypoints outside the compatibility/legacy files.
- `2026-04-18`: `v11.1 Stage 2` control-truth migration is now materially underway: active cancel/approval/reset flows route through `src/mini_agent/runtime/live_control/run_control_store.py`, runtime persistence now records `run_control` + `approval_wait`, and `SessionBackedRunRuntimeAdapter` delegates to direct runtime-manager run methods when available while session detail/read models remain compatibility projections.
- `2026-04-18`: `v11.1 Stage 2` interrupt truth is tightened further: run projections now emit `interrupt_requested` separately from `cancel_requested`, `MainAgentRuntimeManager.interrupt_run()` persists the interrupt request without relabeling it as cancellation, and `SessionBackedRunRuntimeAdapter` now refuses unsupported legacy interrupt paths instead of silently degrading them into cancel requests.
- `2026-04-18`: `v11.1 Stage 2` pause/recovery acknowledgment is now partially landed: turn finalization no longer collapses acknowledged interrupts straight into terminal state, `RuntimeSessionRunControlStore.pause_turn()` keeps run truth paused, and `RuntimeSessionLiveStateHandler` now writes recovery projection payloads for interrupted turns so session/read-model surfaces can expose an honest “paused via next-message recovery” state.

- `2026-04-18`: `v11.1 Stage 2` active transport interrupt exposure is now landed end-to-end: `MainAgentSessionInterruptRequest`, `AgentUserService.interrupt_session_run()`, the gateway `/api/v1/agent/sessions/{session_id}/interrupt` route, `GatewayClient.interrupt_session()`, and `RemoteSessionClient.interrupt_session()` now form one explicit interrupt path with focused gateway/client/application coverage.
- `2026-04-18`: `v11.1 Stage 2` run-facing transport contract has started to become first-class: `MainAgentRunSummary` + `MainAgentRunApprovalWait` now define the typed run DTO surface, gateway exposes `GET /api/v1/agent/runs/{run_id}` and `POST /api/v1/agent/runs/{run_id}/resume`, and transport now includes `GatewayClient.get_run()`, `GatewayClient.resume_run()`, and a typed `RemoteRunClient`.
- `2026-04-18`: `v11.1 Stage 2` run-facing control actions now expand beyond resume: gateway also exposes `POST /api/v1/agent/runs/{run_id}/interrupt`, `.../cancel`, and `.../approval`, while transport and typed run clients now normalize those actions back into `MainAgentRunSummary` so external callers no longer depend on session mutation/approval response shapes for active-run control.
- `2026-04-18`: `v11.1 Stage 2` surface adoption has now started on real entrances: `TUI` and `DesktopUI` gateway bindings both expose typed `RemoteRunClient`, TUI approval/cancel/status rendering now prefers run-level truth over session-recovery inference when available, and desktop approval handling now resolves against run APIs while still keeping session detail as a compatibility read model.
- `2026-04-18`: `v11.1 Stage 2 -> Stage 4` closeout moved another layer of truth out of session compatibility state: runtime read models, persistence metadata, and runtime-policy update gating now all prefer `RuntimeSessionRunControlStore` pending-approval truth over direct `session.runtime.pending_approvals`, while legacy `MainAgentSurfaceService` and `SessionApplicationService` now require an explicit `RunControlApplicationService` instead of treating `AgentUserService` or legacy session methods as implicit run-control owners.
- `2026-04-18`: `v11.1 Stage 4/9` gateway surface routing is now cleaner at the HTTP boundary: `MainAgentRouterDependencies` exposes explicit `get_run_control_service()`, run routes and session cancel/interrupt/approval routes now call `RunControlApplicationService` directly, and gateway composition/router tests now assert that run-control is no longer being tunneled through `AgentUserService`.
- `2026-04-18`: `v11.1 Stage 4` session-scoped compatibility actions are now migrating behind `SessionTaskService`: the service can directly own session runtime policy/context/memory/skill/model-selection actions when its backing runtimes support them, gateway and legacy surface/session facades now prefer that session-task owner first, and older `AgentUserService` / `ModelUserService` entrypoints remain fallback-only compatibility paths instead of the active routing baseline.
- `2026-04-18`: `v11.1 Stage 4` session mutation routing now has a static hygiene guard: `tests/test_session_compat_routing_boundary_hygiene.py` blocks active `src/` code from calling session-scoped compatibility methods on `AgentUserService` / `ModelUserService` outside legacy compatibility owners, so future gateway/surface work cannot silently drift session mutations back onto the wrong services.
- `2026-04-18`: `v11.1 Stage 5` gateway composition now resolves a cached `RuntimeBackedUserServicePorts` bundle first and assembles user services from typed ports instead of assembling directly from the broad `SessionRuntimePort` path at the active entrypoint; the transitional runtime-manager seam remains only inside `resolve_runtime_backed_user_service_ports(...)`.
- `2026-04-18`: `v11.1 Stage 5` typed-port assembly now also has a static hygiene guard: `tests/test_typed_user_service_assembly_boundary_hygiene.py` blocks active `src/` code from calling `assemble_runtime_backed_user_services(...)` outside legacy compatibility assemblies, so new active entrypoints must resolve typed runtime ports first.
- `2026-04-18`: `v11.1 Stage 5` shared user-service assembly no longer imports the broad `SessionRuntimePort` contract directly; runtime-backed typed-port resolution now depends on a narrower structural support protocol, the session-backed compatibility adapters were retargeted to narrower support contracts, and `GatewayComposition.get_session_task_service()` now also consumes the cached typed-port bundle instead of reaching straight into the runtime manager.
- `2026-04-18`: `v11.1 Stage 5` broad session-runtime imports now have a static hygiene guard: `tests/test_session_runtime_port_boundary_hygiene.py` blocks active `src/` code from importing `SessionRuntimePort` outside definition/re-export/legacy compatibility files, so new active assembly work must stay on the narrower runtime-port family.
- `2026-04-18`: `v11.1 Stage 6` workspace-runtime reversibility foothold is now real: `mini_agent.workspace_runtime.snapshot_store` provides the baseline in-memory snapshot owner, `WorkspaceRuntimeBundle` now composes and exposes snapshot capture/readback alongside the mutation ledger, and focused workspace-runtime tests guard the new bundle-level snapshot path.
- `2026-04-18`: `v11.1 Stage 6` workspace-runtime state is now shared and checkpoint-adjacent inside the active host: same-workspace bundles reuse process-local mutation/snapshot state, workspace runtime summaries can surface the latest shared snapshot, and `MainAgentRuntimePersistence.save_session()` now records a `workspace_runtime_snapshot` payload so session persistence begins carrying one real workspace-runtime reversibility hook instead of only sandbox metadata.
- `2026-04-18`: `v11.1 Stage 6` internal snapshot/restore flow now consumes the same workspace-runtime reversibility payload: `RuntimeSessionSnapshot` export/import, hydration payloads, and runtime-state restore all carry `workspace_runtime_snapshot`, while restore rehydrates that payload back into the shared workspace snapshot store so persisted sessions and imported runtime snapshots can restore workspace-runtime reversibility metadata instead of dropping it on the floor.
- `2026-04-18`: `v11.1 Stage 6` developer-facing session diagnostics now expose `workspace_runtime_snapshot` in `MainAgentSessionDetail`/`SessionDetailProjection`, so live and persisted session detail reads can inspect the current reversibility payload without reaching into snapshot import/export internals.
- `2026-04-18`: `v11.1 Stage 6` run control projections now translate `workspace_runtime_snapshot` into a compact `checkpoint` summary on `MainAgentRunSummary`, keeping run APIs aligned to checkpoint semantics (`checkpoint_id`, source, runtime mode/scope, mutation count) instead of leaking the raw workspace-runtime payload shape.
- `2026-04-18`: `v11.1 Stage 7` model-binding truth has started moving off session projection ownership for live runtime reads: `RuntimeSessionModelIdentityCodec.selected_model_identity()` now prefers the attached agent runtime route, and live session read-model / snapshot export builders now consume injected selected/pending model identity callbacks so `selected_model_*` fields are treated as compatibility projections of agent-bound truth instead of the default live source of truth.
- `2026-04-18`: `v11.1 Stage 7 -> Stage 9` TUI live-session model rendering now follows the same truth rule: `MiniAgentTuiApp._session_selected_model_identity()` prefers the attached runtime route before falling back to session projection fields, so local model hint/usage paths stop reintroducing stale projection-first behavior after the runtime-side model-truth migration.
- `2026-04-18`: `v11.1 Stage 7` persistence writeout now follows the same model-truth rule: `RuntimeSessionPersistenceRecordBuilder` accepts selected/pending model identity callbacks, `MainAgentRuntimeManager` wires those callbacks to `RuntimeSessionModelIdentityCodec`, and persisted session records therefore prefer live agent-bound model identity over stale `session.projection.selected_model_*` compatibility fields when saving active sessions.
- `2026-04-18`: `v11.1 Stage 10` model-binding compatibility imports were tightened another step: active `ModelBindingApplicationService` and `ModelUserService` now depend on the stable `application.ports.session_model_selection_runtime_port` wrapper instead of importing the legacy module directly, and `tests/test_session_model_selection_runtime_port_boundary_hygiene.py` prevents future active source drift back to the legacy port owner.
- `2026-04-18`: `v11.1 Stage 10` agent compatibility imports were tightened in the same way: active `AgentApplicationService` and `AgentUserService` now depend on `application.ports.session_agent_runtime_port` rather than the legacy module directly, and `tests/test_session_agent_runtime_port_boundary_hygiene.py` blocks future active imports from drifting back to the legacy port owner.
- `2026-04-18`: `v11.1 Stage 10` user-service assembly no longer imports `legacy.session_runtime_compat` directly: the active assembly path now consumes `application.user_services.session_runtime_compat_adapters`, the root `application.session_runtime_compat` wrapper re-exports through that stable owner for the run/task adapters, and `tests/test_session_runtime_compat_legacy_boundary_hygiene.py` blocks future active source drift back to the legacy compat module.
- `2026-04-18`: `v11.1 Stage 10` run/task session-runtime compatibility implementation now lives in the stable `application.user_services.session_runtime_compat_adapters` owner instead of `legacy.session_runtime_compat`; the legacy module only re-exports those adapters while retaining the still-legacy agent/model compatibility pieces, and the related namespace/legacy hygiene tests now also catch relative-import drift instead of only absolute imports.
- `2026-04-18`: `v11.1 Stage 10` session-runtime compatibility implementations are now fully owned by `application.user_services.session_runtime_compat_adapters`: both root `application.session_runtime_compat` and legacy `application.legacy.session_runtime_compat` have been reduced to re-export wrappers, so compatibility entrypoints remain available without keeping active adapter implementation inside the legacy module.
- `2026-04-18`: `v11.1 Stage 10` surface/session compatibility wrapper modules are now lazy shims instead of eager re-export modules: root and facade wrappers for `main_agent_surface_service`, `surface_service_assembly`, `session_service`, `session_service_assembly`, and `surface_dependency_resolution` now resolve compatibility symbols only on access, and `tests/test_application_compat_wrapper_lazy_exports.py` guards that lazy export behavior.
- `2026-04-18`: `v11.1 Stage 10` compatibility shim normalization was extended further: `application.user_service_assembly` and `application.session_runtime_port` now also lazy-resolve their compatibility exports instead of eagerly importing the underlying owners at module import time.
- `2026-04-18`: `v11.1 Stage 10` root `application/` compatibility modules were normalized further across `agent_*`, `channel_*`, `operations_*`, and `surface/support` wrappers: those modules now use the same lazy shim pattern as the larger surface/session wrappers, including lazy access for compatibility imports that point outside `application/` such as provider/model helper exports.
- `2026-04-18`: `v11.1 Stage 10` shim normalization now also covers the remaining compatibility edge wrappers: `application.session_runtime_compat`, `application.ports.session_agent_runtime_port`, and `application.ports.session_model_selection_runtime_port` now lazy-resolve their compatibility exports instead of importing those legacy/stable owners eagerly at module import time.
- `2026-04-18`: `v11.1 Stage 10` runtime compatibility wrapper normalization has now started on the main session-management family: root `mini_agent.runtime` wrappers for access/admin/creation/agent-runtime/control-model/read-model/snapshot/restore/state-hydrator/model-identity/payload-codec now lazy-resolve their underlying owners, and `tests/test_runtime_compat_wrapper_lazy_exports.py` guards that runtime-side lazy export behavior.
- `2026-04-18`: `v11.1 Stage 10` runtime shim normalization now also covers the main service/support family: root `mini_agent.runtime` wrappers for cancel/interrupt/pending-approval/lifecycle/policy/sandbox/lineage/path-utils/turn-context/policy-loader now lazy-resolve their underlying owners instead of importing them eagerly at module import time.
- `2026-04-19`: `v11.1` agent-core physical alignment tightened again: the non-target root owner `src/mini_agent/agent_core/kernel_state.py` has been hard-cut, kernel truth bundle/serde helpers now live under `src/mini_agent/agent_core/contracts/_kernel_state_bundle.py`, runtime orchestration consumes those helpers through `mini_agent.agent_core.contracts`, and `tests/test_v11_1_agent_core_root_owner_hard_cut.py` now blocks the deleted root owner from returning.
- `2026-04-19`: `v11.1` runtime physical alignment tightened again: deleted root runtime wrappers `src/mini_agent/runtime/session_diagnostics_service.py` and `src/mini_agent/runtime/session_runtime_persistence.py` are now gone, `src/mini_agent/runtime/__init__.py` is reduced to a package marker, active source no longer imports the runtime package root, and `tests/test_v11_1_runtime_root_owner_hard_cut.py` plus the strengthened runtime namespace hygiene test block those root owners from returning.
- `2026-04-19`: `v11.1` package-root facade cleanup advanced across `commands/`, `tools/`, and `workspace_runtime/`: their `__init__.py` files are now package markers instead of active export facades, active source imports concrete owners directly, and `tests/test_command_namespace_boundary_hygiene.py`, `tests/test_tools_namespace_boundary_hygiene.py`, and `tests/test_workspace_runtime_namespace_boundary_hygiene.py` now guard against package-root drift.
- `2026-04-19`: the package-root cleanup also exposed a real structural issue rather than just import style drift: eager package facades had been hiding circular-import risk across `commands -> tools -> workspace_runtime -> runtime/agent_core`; after the hard cut, that chain now resolves through direct owner imports and the full suite remains green.
- `2026-04-18`: `v11.1 Stage 10` runtime root wrapper normalization now covers the remaining control/support/orchestration/persistence compatibility modules as well: interaction-surface, catalog/operator/memory/MCP handlers, hydration/store/persistence/snapshot/turn-scope helpers, related local-runtime wrappers, and the `tooling.py` compatibility bridge now all use lazy export resolution, while `tests/test_runtime_compat_wrapper_lazy_exports.py` enforces the `_COMPAT_EXPORTS` lazy-shim rule across the full root runtime compatibility surface.
- `2026-04-18`: `v11.1 Stage 10` runtime namespace drift now has a matching hygiene guard: `tests/test_runtime_namespace_boundary_hygiene.py` blocks active `src/` code from importing `mini_agent.runtime.*` root compatibility wrappers directly, so new runtime work must depend on the stable `handlers/`, `support/`, `orchestration/`, `live_control/`, or `read_models/` owners instead of the compatibility layer.
- `2026-04-18`: `v11.1 Stage 10` compatibility-wrapper shape is now guarded repo-wide: `tests/test_compat_wrapper_shape_hygiene.py` scans every `src/` compatibility re-export module and requires the shared lazy-wrapper contract (`_COMPAT_EXPORTS`, `__getattr__`, `__dir__`), so future cleanup or new compatibility files cannot silently reintroduce eager wrapper shapes in another package.
- `2026-04-18`: `v11.1 Stage 10` surface dependency-resolution compatibility access is now guarded explicitly: `tests/test_surface_dependency_resolution_boundary_hygiene.py` blocks active source from depending on `application.facades.surface_dependency_resolution`, keeping those helper functions confined to the legacy surface assembly boundary.
- `2026-04-18`: `v11.1` hard-alignment `Stage H2` has now entered real code: `src/mini_agent/agent_core/contracts/` gained first-class `AgentProfile`, `AgentInstance`, `Run`, `WorkspaceAttachment`, `SessionAttachment`, `CapabilitySnapshot`, `Checkpoint`, and `ExecutionJournal` contracts beside the earlier `RunControlState` and `ApprovalWait`, and `tests/test_v11_1_stage_h2_kernel_contracts.py` now guards the new kernel object family directly.
- `2026-04-19`: `v11.1 Stage H4` hard cut is now materially landed: `MainAgentSurfaceService`, `SessionApplicationService`, their root/facade wrappers, `application.session_runtime_compat`, and the whole `application.legacy/` tree were physically removed from the active repo; walkthrough scripts now compose `SessionTaskService + RunControlApplicationService + AgentInteractionApplicationService` directly, and `tests/test_v11_1_stage_h4_application_hard_cut.py` plus the updated boundary hygiene tests guard against those deleted owners returning.
- `2026-04-18`: `v11.1 Stage 9` real surface work has resumed on the remote-stream path: TUI remote turn consumption now surfaces gateway `thinking_delta` events into the activity block without mixing them into the main chat transcript, and DesktopUI now normalizes the same event into updateable activity cards so remote reasoning/progress can stay visible in the Activity panel without spamming duplicate entries.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI session/run action flow is now thinner and testable: share/fork/approval mutation assembly lives in dedicated desktop window helpers, run-level approval truth is preferred when available, and `tests/test_desktop_window_session_actions.py` guards the real service-contract behavior instead of leaving it buried only inside Qt event handlers.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI session action coverage expanded further: rename/compact/create now follow the same helper-owned path as share/fork/approval, including derived-session creation vs fresh workspace session creation, so the remaining desktop session management flow is no longer trapped only inside `DesktopMainWindow` event handlers.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI prompt submission contract is now guarded too: desktop chat submission now builds a typed `MainAgentChatRequest` through `build_desktop_chat_request(...)`, the missing `MainAgentChatRequest` import path was corrected, and helper coverage now exercises the real `session_id / workspace_dir / surface=desktop` binding so desktop send-flow regressions are caught before manual UI testing.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI stream transcript handling is now helper-owned and test-covered too: outgoing prompt recording, assistant chunk aggregation, stream completion hydration, and stream failure cleanup now route through dedicated desktop helpers, including clearing stale `streaming` flags on failure so the desktop transcript state stays consistent after interrupted runs.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI now exposes a real stop-turn control on top of the run-level contract: the composer includes a `Stop` action, desktop command palette adds `Stop Turn`, button enablement follows run busy/approval truth instead of raw widget state, and focused tests now guard the `cancel_run_sync` path and cancel-affordance rule.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI run-control affordances now cover the full basic trio: `Pause`/`Resume` were added beside `Stop`, command palette now exposes `Pause Turn` and `Resume Turn`, enablement follows explicit run-summary truth (`busy`, `waiting_on_approval`, `interrupt_requested`, `cancel_requested`, `resumable`), and focused tests now guard both the interrupt and resume typed run-contract paths.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI chat-header run state now follows run truth instead of a plain busy flag: the header badge can now distinguish `Waiting / Interrupting / Cancelling / Busy / Interrupted / Ready`, and helper coverage guards the precedence so desktop status feedback stays aligned with run-level control state.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI chat context panel now surfaces run read-model detail directly: the context view appends compact run summary lines for `status / phase / control / running_state`, plus approval-wait and checkpoint snippets when present, so run-level truth is visible in the main chat workspace instead of only affecting button enablement.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI approvals visibility is now stronger: the top `Approvals` action shows a live pending count, and that count prefers run-level approval-wait truth over raw session pending lists so the main chat header stays aligned with the active run contract.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI `Sessions` page now reflects run truth too: the session detail inspector appends the same compact run summary block used by chat context, so session-history inspection shows current run `status / phase / control / approval wait / checkpoint` instead of only session-era metadata.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI session rows are now more scan-friendly: left-rail/session-list rows no longer flatten everything into `Busy`, instead preferring `Waiting / Cancelling / Interrupting / Interrupted` when those states are visible from session summary fields, while still preserving the `Shared` marker.
- `2026-04-18`: `v11.1 Stage 9` DesktopUI chat workspace now exposes a visible `Runtime` tab instead of keeping runtime text hidden; that panel shows selected model inventory plus the compact run summary block, and its empty/error states were corrected so it no longer leaks stale run text when no session is selected or detail loading fails.
- `2026-04-18`: `mini_agent.application` now treats session-era namespace exports like the surface-facade exports: `SessionApplicationService`, session-service assembly helpers, and broad session-runtime root symbols are lazy compatibility exports only, no longer promoted through the active package `__all__`, and `tests/test_session_namespace_boundary_hygiene.py` blocks active `src/` code from importing those compatibility entrypoints from the root package or wrapper modules.
- `2026-04-18`: `mini_agent.application.user_services` now follows the same rule for the runtime-backed assembly shortcut: `assemble_runtime_backed_user_services(...)` is no longer promoted through the active `application` / `user_services` package `__all__` surfaces, remaining available only through compatibility resolution, while `GatewayComposition` now imports typed assembly helpers from the stable `user_services` package instead of the `user_service_assembly` compatibility wrapper.
- `2026-04-18`: `v11.1 Stage 5` user-service assembly namespace drift now has a dedicated hygiene guard: `tests/test_user_service_assembly_namespace_boundary_hygiene.py` blocks active `src/` code from importing `mini_agent.application.user_service_assembly`, so future active wiring must use the stable `user_services` package or deeper typed owners instead of the compatibility re-export module.
- `2026-04-18`: `AgentModelRuntimeAdapter` now lives in the stable `mini_agent.application.user_services.model_runtime_adapter` owner; `GatewayComposition` uses that stable module directly, while `mini_agent.application.session_runtime_compat` remains a compatibility re-export only.
- `2026-04-18`: active `src/` code no longer depends on the root `mini_agent.application.session_runtime_compat` wrapper, and `tests/test_session_runtime_compat_namespace_boundary_hygiene.py` blocks future active imports from that wrapper so the stable model adapter owner stays separate from legacy session compatibility adapters.
