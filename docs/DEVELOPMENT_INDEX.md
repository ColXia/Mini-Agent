...
- `2026-04-18`: `v11.1 Stage 10` runtime root wrapper normalization now covers the remaining control/support/orchestration/persistence compatibility modules as well: interaction-surface, catalog/operator/memory/MCP handlers, hydration/store/persistence/snapshot/turn-scope helpers, and related local-runtime wrappers now all use `_COMPAT_EXPORTS` lazy shims, while `tests/test_runtime_compat_wrapper_lazy_exports.py` enforces that every root runtime compatibility wrapper except the special `tooling.py` bridge stays on the lazy-export pattern.
