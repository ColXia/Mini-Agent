...
### 改造后架构 (详见 `docs/TRANSFORMATION_PLAN.md`)

改造后的 Mini-Agent 将融合 7 个开源项目的核心能力，包括：
- **记忆核心**: Memoria STM/LTM + Codex 两阶段巩固 + Gemini GEMINI.md + Hermes FTS5 搜索
- **模型管理**: CC Switch 自定义 Provider + 熔断器 + 故障转移
- **编程代理**: Codex 事件循环 + Windows 沙箱 + Gemini DeclarativeTool + extracted-src 协调器
- **智能体核心**: OpenClaw 8 级路由 + 技能平台 + 定时任务 + Hermes 子 Agent 委托
...
class SessionNoteTool:
    ...
    async def execute(self, content: str, category: str = "general", scope: str = "both"):
        # 写入 MEMORY.md 和/或 memory/YYYY-MM-DD.md
        ...
