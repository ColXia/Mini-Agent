"""Channel-facing novel action parsing and dispatch."""
...
from mini_agent.novel import NovelServiceUseCases

ResolveWorkspaceDirFn = Callable[[str | None], Path]
ToUtcIsoFn = Callable[[datetime], str]


class ChannelNovelActionHandler:
    """Owns remote-channel `/novel ...` action parsing and execution."""
    ...
    async def maybe_handle(self, request: ChannelMessageRequest) -> ChannelMessageResponse | None:
        novel_action = self._extract_novel_action(request)
        if novel_action is None:
            return None
