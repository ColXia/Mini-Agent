"""Workspace-skill catalog and policy helpers shared across surfaces."""
...
from mini_agent.agent_core.skills.path_resolver import (
...
def _require_skill_config(config: Any | None) -> Any:
    if config is None:
        raise ValueError(
            "Skill catalog config is required when the active agent does not expose a skill loader."
        )
    return config
...
def resolve_skill_catalog_loader(
    *,
    ...
    active_config = _require_skill_config(config)
