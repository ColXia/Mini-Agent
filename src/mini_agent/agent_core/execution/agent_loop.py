"""Submission-loop baseline for agent-core execution event processing."""
...
from mini_agent.agent_core.context.loop_context import AgentLoopContext, TurnContext
from mini_agent.agent_core.execution.scheduler import (
    SchedulerResult,
    SchedulerState,
    TurnScheduler,
)
from mini_agent.agent_core.runtime_bindings import set_agent_tool_approval_handler
...
class _TurnProgressRecorder:
    ...
    def record_llm_event(self, step: int, event: Any) -> dict[str, Any]:
        event_type = getattr(getattr(event, "type", None), "value", None) or _safe_text(getattr(event, "type", None))
        ...
        if isinstance(delta, str) and delta:
            payload["delta"] = delta
        finish_reason = _safe_text(getattr(event, "finish_reason", None))
        if finish_reason:
        ...
        if tool_call is not None:
            payload["tool_call_id"] = _safe_text(getattr(tool_call, "id", None))
            function = getattr(tool_call, "function", None)
            payload["tool_name"] = _safe_text(getattr(function, "name", None))
        return payload
...
class AgentSubmissionLoop:
    ...
    async def _ensure_agent(self) -> Any:
        ...
        async with self._agent_lock:
            ...
            try:
                set_agent_tool_approval_handler(created, self._request_exec_approval)
            except Exception as exc:
                raise RuntimeError(
                    "submission loop could not bind the tool approval handler"
                ) from exc
            self._agent = created
            return self._agent
    ...
    def _compose_hooks(self, external_hooks: Any | None, progress: _TurnProgressRecorder) -> Any:
        ...
        async def _on_llm_event(step: int, event: Any) -> None:
            await self._publish("loop.llm_event", progress.record_llm_event(step, event))
            await _call_hook(getattr(external_hooks, "on_llm_event", None), step, event)
        ...
        try:
            try:
                ...
                result = SchedulerResult(
                    ...
                    message=f"Agent bootstrap failed: {type(exc).__name__}: {exc}",
                    error="agent_bootstrap_failed",
                )
                agent = None
            else:
