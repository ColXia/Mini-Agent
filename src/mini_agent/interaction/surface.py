...
USER_ENTRANCES: tuple[str, ...] = ("cli", "tui", "desktop", "remote")
# The active codebase currently carries exactly one maintained remote adapter.
ACTIVE_REMOTE_CHANNEL_ADAPTERS: tuple[str, ...] = ("qq",)
...
def resolve_remote_channel(surface: str | None, channel_type: str | None) -> str | None:
    """Resolve the active concrete remote adapter identity, when one exists."""

    normalized_surface = normalize_surface_label(surface)
    normalized_channel = normalize_channel_type(channel_type)
    if normalized_channel in ACTIVE_REMOTE_CHANNEL_ADAPTERS:
        return normalized_channel
    if normalized_surface in ACTIVE_REMOTE_CHANNEL_ADAPTERS:
...
def resolve_user_entrance(surface: str | None, channel_type: str | None) -> str:
    ...
    if normalized_surface in _REMOTE_ENTRANCE_ALIASES:
        return "remote"
    if normalized_channel and (not _clean(surface) or normalized_surface == normalized_channel):
        return "remote"
    return "cli"
...
def resolve_interaction_surface(surface: str | None, channel_type: str | None) -> InteractionSurface:
    ...
    normalized_surface = normalize_surface_label(surface)
    normalized_channel = normalize_channel_type(channel_type)
    if not _clean(surface) and normalized_channel in ACTIVE_REMOTE_CHANNEL_ADAPTERS:
...
def resolve_interaction_binding(
    *,
    ...
    if candidate_surface:
        ...
        entrance = None
        remote_channel = normalized_channel if normalized_channel in ACTIVE_REMOTE_CHANNEL_ADAPTERS else None
