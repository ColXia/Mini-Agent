...
def test_remote_session_client_ensure_default_session_returns_typed_detail() -> None:
    async def _run() -> None:
        service = RemoteSessionClient(session_transport=_DummyGatewayClient())

        detail = await service.ensure_default_session(
            MainAgentDefaultSessionRequest(
        ...
        assert detail.session_id == "default"
        assert detail.is_default is True
        assert detail.origin_surface == "desktop"
        assert detail.channel_type == "qq"
        assert detail.conversation_id == "group:demo"
        assert detail.sender_id == "user-1"
