"""Tests for P14 T2.6 code-agent MCP client baseline."""
...
from mini_agent.code_agent import CodeAgentMCPClient, infer_mcp_tool_attributes
from mini_agent.code_agent.mcp_tools import mcp_tool_alias
...
@pytest.mark.asyncio
async def test_code_agent_mcp_client_connects_lists_and_calls():
    client = CodeAgentMCPClient(
...
@pytest.mark.asyncio
async def test_code_agent_mcp_client_builds_namespaced_declarative_registry():
    client = CodeAgentMCPClient(
        config_path="mcp.json",
        discover_fn=_discover_stub,
        connection_factory=_FakeConnection,
