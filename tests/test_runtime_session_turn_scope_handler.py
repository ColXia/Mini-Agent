...
def test_turn_scope_handler_pending_approval_wrappers_use_default_support_owner() -> None:
    handler = RuntimeSessionTurnScopeHandler()
    ...
    assert RuntimeSessionTurnScopeHandler.normalize_pending_approval({"token": "tok-2"}) == {
        "token": "tok-2",
        "tool_name": "tool",
        "arguments": {},
        "kind": None,
    ...
    assert RuntimeSessionTurnScopeHandler.pending_approvals_from_raw(
...
def test_turn_scope_handler_legacy_recovery_wiring_builds_and_clears_context() -> None:
...
def test_turn_scope_handler_legacy_recovery_wiring_resets_runtime_state() -> None:
    session = _session_for_recovery()
...
def test_turn_scope_handler_mark_turn_finished_pauses_interrupt_requested_runs() -> None:
    session = _session_for_pending()
...
def test_turn_scope_handler_prefers_injected_support_owners() -> None:
    class _PendingState:
        def __init__(self) -> None:
...
def test_turn_scope_handler_supports_turn_scope_enter_exit_on_target_owner() -> None:
    session = _session_for_pending()
    session.runtime.lock = asyncio.Lock()
    session.projection.active_surface = "desktop"
    session.projection.origin_surface = "desktop"
    ...
    class _RecoveryReset:
        def build_recovery_turn_context(self, session):  # noqa: ANN001
            return {"session_id": session.session_id, "resume_strategy": "target-owner"}

    async def _ensure_agent_model_binding(target) -> bool:  # noqa: ANN001
        lifecycle_calls.append(("ensure", target.session_id))
        return True

    async def _apply_skill_reload(target) -> bool:  # noqa: ANN001
        lifecycle_calls.append(("reload", target.session_id))
        return True

    handler = RuntimeSessionTurnScopeHandler(
        recovery_reset=_RecoveryReset(),
        ensure_agent_model_binding_for_turn_fn=_ensure_agent_model_binding,
        apply_pending_session_skill_reload_fn=_apply_skill_reload,
        persist_session_fn=lambda target: persist_calls.append(target.session_id),
    )

    async def _run() -> None:
        recovery = await handler.enter(
            session,
            surface="desktop",
        ...
        assert recovery == {
            "session_id": session.session_id,
            "resume_strategy": "target-owner",
        }
        assert session.runtime.lock.locked() is True

        await handler.exit(session)

    asyncio.run(_run())

    assert session.runtime.lock.locked() is False
    assert lifecycle_calls == [
        ("ensure", session.session_id),
        ("reload", session.session_id),
        ("reload", session.session_id),
    ]
    assert persist_calls == [session.session_id] * 5


def test_turn_scope_handler_capture_prepared_context_persists_on_target_owner() -> None:
    session = _session_for_pending()
    captured: list[str] = []
    restored: list[str] = []
    persisted: list[str] = []
    handler = RuntimeSessionTurnScopeHandler(
        capture_prepared_context_state_mutation=lambda target: captured.append(target.session_id),
        restore_prepared_context_state_mutation=lambda target: restored.append(target.session_id),
        persist_session_fn=lambda target: persisted.append(target.session_id),
    )

    handler.capture_prepared_context_state(session)
    handler.restore_prepared_context_state(session)
