"""Unit tests for agent-core execution policy and tool-call budgeting."""
...
class OverflowThenSuccessLLM:
    ...
    async def generate(self, messages, tools=None):  # noqa: ANN001,ARG002
        ...
        return LLMCompletionResult(content="done", thinking=None, tool_calls=None, finish_reason="stop")
...
def test_agent_override_execution_policy_preserves_typed_policy(tmp_path: Path) -> None:
    logger = AgentLogger(log_dir=tmp_path / "logs")
    agent = Agent(
        llm_client=SequenceLLM([LLMCompletionResult(content="done", thinking=None, tool_calls=None, finish_reason="stop")]),
    ...
    original_policy = agent.execution_policy
    with agent.override_execution_policy({"max_steps": 2, "max_tool_calls_per_step": 1}):
        assert isinstance(agent.execution_policy, AgentExecutionPolicy)
        assert agent.execution_policy.max_steps == 2
        assert agent.execution_policy.max_tool_calls_per_step == 1
        assert agent.max_steps == 2
        assert agent.max_tool_calls_per_step == 1

    assert agent.execution_policy == original_policy
    assert isinstance(agent.execution_policy, AgentExecutionPolicy)
    assert agent.max_steps == 4
    assert agent.max_tool_calls_per_step == 3
...
async def test_executor_returns_complete_transition_without_tools(tmp_path: Path):
    logger = AgentLogger(log_dir=tmp_path / "logs")
    agent = Agent(
        llm_client=SequenceLLM([LLMCompletionResult(content="", finish_reason="stop")]),
        system_prompt="system",
        tools=[EchoTool()],
        max_steps=2,
        workspace_dir=str(tmp_path / "workspace"),
