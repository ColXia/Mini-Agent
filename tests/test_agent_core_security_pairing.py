...
def test_pairing_store_limit_expiry_and_approval_flow(tmp_path):
    ...
    one = store.upsert_request(channel="wechat", entry_id="u-1", now_utc=base)
    two = store.upsert_request(channel="wechat", entry_id="u-2", now_utc=base + timedelta(seconds=1))

    with pytest.raises(PairingLimitError):
        store.upsert_request(channel="wechat", entry_id="u-3", now_utc=base + timedelta(seconds=2))

    pending_before = store.list_pending(channel="wechat", now_utc=base + timedelta(seconds=3))
    assert [item.entry_id for item in pending_before] == ["u-1", "u-2"]

    approved = store.approve_code(channel="wechat", code=one.code, now_utc=base + timedelta(seconds=4))
    assert approved is not None
    assert approved.entry_id == "u-1"
    assert store.is_allowed(channel="wechat", entry_id="u-1") is True

    pending_after = store.list_pending(channel="wechat", now_utc=base + timedelta(seconds=4))
    assert [item.entry_id for item in pending_after] == ["u-2"]
    assert store.list_pending(channel="wechat", now_utc=base + timedelta(seconds=15)) == ()
