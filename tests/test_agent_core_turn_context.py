...
async def test_run_turn_injects_ephemeral_context_without_polluting_history(tmp_path: Path) -> None:
    llm = _CaptureLLM([LLMCompletionResult(content="done", thinking=None, tool_calls=None, finish_reason="stop")])
...
@pytest.mark.asyncio
async def test_prepared_turn_context_service_orchestrates_providers_and_emits_context_message(
    tmp_path: Path,
) -> None:
    recorded_events: list[tuple[str, str, dict[str, object]]] = []
    logger = SimpleNamespace(
        log_event=lambda event_type, payload, level="info": recorded_events.append(
            (str(event_type), str(level), dict(payload))
        )
    )
    service = AgentPreparedTurnContextService(
        workspace_dir=tmp_path / "workspace",
        providers=[_StaticProvider(), _FailingProvider()],
        logger=logger,
        context_message_name_prefix="__prepared_ctx__",
    )

    result = await service.prepare_turn_context(
        turn_context=RuntimeTurnContext(
            session_id="sess-service",
            submission_id="sub-service",
            user_input="inject context via service",
        ),
    ...
    assert result.summary["sources"] == ["static"]
    assert result.context_message is not None
    assert result.context_message.name == "__prepared_ctx__:sub-service"
    assert "Injected context" in str(result.context_message.content)
    assert result.diagnostics["turn_count"] == 1
    assert result.diagnostics["turns_with_context"] == 1
    assert any(event_type == "turn_context.provider_failed" for event_type, _level, _payload in recorded_events)
    assert any(event_type == "turn_context.prepared" for event_type, _level, _payload in recorded_events)
