...
def test_use_case_tracks_shared_session_metadata_and_recent_messages() -> None:
    async def _run() -> None:
        ...
        activated = await _activate_runtime_surface(use_cases, "sess-qq", surface="tui")
...
def test_use_case_can_list_and_refresh_shared_session_skills(tmp_path: Path, monkeypatch) -> None:
...
def test_use_case_can_manage_shared_session_skill_policy(tmp_path: Path, monkeypatch) -> None:
    builtin_dir = tmp_path / "builtin-skills"
    workspace_dir = tmp_path / "workspace"
    workspace_skill_dir = workspace_dir / ".mini-agent" / "skills"
    builtin_dir.mkdir(parents=True, exist_ok=True)
...
def test_use_case_can_install_workspace_skill_for_shared_session(tmp_path: Path, monkeypatch) -> None:
    builtin_dir = tmp_path / "builtin-skills"
    workspace_dir = tmp_path / "workspace"
...
def test_use_case_returns_not_found_for_missing_shared_session_skill(tmp_path: Path, monkeypatch) -> None:
    builtin_dir = tmp_path / "builtin-skills"
    workspace_dir = tmp_path / "workspace"
    builtin_dir.mkdir(parents=True, exist_ok=True)
    (builtin_dir / "doc-coauthoring").mkdir(parents=True, exist_ok=True)
    ...
    async def _run() -> None:
        runtime = MainAgentRuntimeManager(ttl_seconds=3600, build_agent=lambda _workspace: asyncio.sleep(0, result=_SelectableAgent(provider_source="preset", provider_id="openai", model_id="gpt-5.4")))
        use_cases = MainAgentGatewayUseCases(
            runtime_manager=runtime,
            resolve_workspace_dir=lambda value: Path(str(value or workspace_dir)).resolve(),
            to_utc_iso=_to_utc_iso,
...
def test_use_case_rejects_invalid_shared_session_skill_mode(tmp_path: Path, monkeypatch) -> None:
    builtin_dir = tmp_path / "builtin-skills"
    workspace_dir = tmp_path / "workspace"
    builtin_dir.mkdir(parents=True, exist_ok=True)
    (builtin_dir / "doc-coauthoring").mkdir(parents=True, exist_ok=True)
...
def test_use_case_reports_shared_session_skill_support_disabled(tmp_path: Path, monkeypatch) -> None:
    workspace_dir = tmp_path / "workspace"
...
def test_use_case_persisted_interrupted_session_exposes_recovery_snapshot_after_restart(tmp_path: Path) -> None:
    async def _run() -> None:
        ...
        runtime_second = MainAgentRuntimeManager(
            ttl_seconds=3600,
            build_agent=_build_agent,
            storage_dir=tmp_path / "recovery-store",
        )
        use_cases_second = MainAgentGatewayUseCases(
            runtime_manager=runtime_second,
            resolve_workspace_dir=_resolve_workspace_dir,
            to_utc_iso=_to_utc_iso,
            sse_event=_sse_event,
            format_bootstrap_error=_format_bootstrap_error,
...
def test_use_case_restarted_shared_session_keeps_recovery_until_next_turn_consumes_it(tmp_path: Path) -> None:
    async def _run() -> None:
        ...
        activated = await _activate_runtime_surface(use_cases_second, "sess-recovery-continue", surface="tui")
        assert activated.active_surface == "tui"
...
def test_use_case_control_session_mcp_list_records_operator_snapshot(monkeypatch) -> None:
    async def _run() -> None:
        ...
        monkeypatch.setattr(
            "mini_agent.runtime.main_agent_runtime_manager.Config.load",
            lambda allow_interactive_setup=False: object(),
        )
        monkeypatch.setattr(
...
def test_use_case_chat_without_session_id_restores_latest_persisted_shared_session(tmp_path: Path) -> None:
    async def _run() -> None:
        async def _build_agent(_workspace: Path):
            return _DummyAgent()
        ...
        assert chat.session_id == "sess-new"
        detail = await use_cases.get_session_detail("sess-new", recent_limit=10)
        assert detail.title == "Session 2"
        assert detail.active_surface == "qq"
        assert detail.reply_enabled is True
...
def test_use_case_chat_applies_session_title_hint_on_new_shared_session(tmp_path: Path) -> None:
    async def _run() -> None:
        async def _build_agent(_workspace: Path):
            return _DummyAgent()
...
def test_use_case_routing_diagnostics_tracks_hits_cache_and_fallback() -> None:
    async def _run() -> None:
        build_calls = 0
