...
from mini_agent.agent import Agent, TurnExecutionResult, TurnStopReason
from mini_agent.code_agent import ApprovalEngine, PermissionPolicy, estimate_tokens
...
async def test_submission_loop_completed_payload_includes_prepared_turn_context(tmp_path):
    ...
    agent = Agent(
        llm_client=_SequenceLLM([LLMResponse(content="done", thinking=None, tool_calls=None, finish_reason="stop")]),
        system_prompt="system",
        tools=[],
        max_steps=2,
        workspace_dir=str(tmp_path / "workspace"),
