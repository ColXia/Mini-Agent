param(
    [string]$ChannelDir = "C:\Users\Conli\Mini-Agent\channels\qqbot"
)

$ErrorActionPreference = "Stop"

if (-not (Test-Path $ChannelDir)) {
    throw "QQ channel directory not found: $ChannelDir"
}

$envPath = Join-Path $ChannelDir ".env"
if (-not (Test-Path $envPath)) {
    Write-Host "[Warn] .env not found. Please copy .env.example to .env and fill QQBOT_APPID/QQBOT_SECRET first." -ForegroundColor Yellow
}

$existing = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -eq "node.exe" -and $_.CommandLine -match "channels\\qqbot\\dist\\index\.js"
}
...
Write-Host "QQ Bot channel start command sent." -ForegroundColor Green
Write-Host "Dir: $ChannelDir" -ForegroundColor DarkGray
Write-Host "Log: $ChannelDir\runtime.log" -ForegroundColor DarkGray
